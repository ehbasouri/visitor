...
function UpdateUserInfo({history}) {
    ...
    const [localSign, setLocalSign] = useState(HOST + user_info.sign_url);
    ...
    async function updateProduct() {
        ...
        try {
            ...
            if(sign_url){
                const signUploadResponse = await API.multipartPost(sign_url);
                userData.sign_url = signUploadResponse.data.filename;
            }
            const { data } = await API.put("business", userData);
            Auth.signIn(data);
    ...
    function onChangeSign(e) {
        if (e.target.files && e.target.files[0]) {
            setSign_url(e.target.files[0]);
            setLocalSign(URL.createObjectURL(e.target.files[0]))
        }
    }
        ...
        <div className={"mainScreen"}>
            ...
            <div className={"mainContainer"} >
                <MainScreen>
                    ...
                    <div className={classes.imageContainer} >
                        <input accept="image/*" onChange={onChangeSign} className={classes.input} name={"file"} id="icon-button-sign" type="file" />
                        <label htmlFor="icon-button-sign">
                            <IconButton color="primary" aria-label="upload picture" component="span">
                            <PhotoCamera />
                            </IconButton>
                        </label>
...
export default SceneWrapper(UpdateUserInfo);